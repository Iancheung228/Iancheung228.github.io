...
In ab testing, we are interested in determining whether the treatment mean or the control mean is different. Many times we can't observe the individual under both treatment and control and we depend on randomization. There is another practical concern: we will never be able to involve the entire population, we can only sample a subset of the population. 

sample a subset of population
randomly assign treatment and control
calculate the mean under treatment $$Y^T$$ and the mean under control $$Y^C$$,this will be unbiased estimate of the population treatment mean and population control mean  
calculate $$ \Delta = Y^T - Y^C $$. 
because we calculated the mean over a subset of population, a new sample of the population will likely result in a different mean, hence variability
we can think of the sampled mean as a particular instance of the sampling distribution (can mention CLT)

Under the Null hypothesis, we assume the true $$ \Delta $$ is 0
as discussed earlier there the sampling distribution will be a normal distribution centered at 0

$$ \frac{MLE - truth}{appropriate s.d.} $$
