...
$$ \text{p-value} = P( |Z| \geq z_{\alpha/2} |\text{Null hyp is assumed to be true}) = P(| \zeta | \geq z_{\alpha/2}) = \alpha = 5\% $$
...
$$ \text{power} =  P(\text{p-value} < \alpha | Alternative hyp is assumed to be true) $$
