...
In ab testing, we have 2 versions of a product, one is currently in production and the other is the contender. We wish to determine whether there is a difference between the control vs treatment on the metric we are interested in. Most times we can't observe the individual under both treatment and control and we depend on randomization. There is another practical concern: we will rarely conduct the experiment over the entire population, to which we sample a subset of the population for the experiment. 

1. Sample a subset of the population
2. Randomly assign treatment and control
3. Calculate the mean under treatment $$Y^T$$ and the mean under control $$Y^C$$. They will be the unbiased estimate of the population treatment mean and population control mean  
4. Calculate the difference in sample mean $$ \Delta = Y^T - Y^C $$. 
Note, due to step 1, each time we repeat the experiment the observed sample mean and consequently the sample difference in mean will likely vary, a.k.a variability
****we can think of the sampled mean as a particular instance of the sampling distribution (can mention CLT)****

Based on the observed mean (one instance), how unlikely is it to come from the null distribution? If it is extremely unlikely our observed mean is generated by the null distribution, we are likely to suspect the Null hypothesis is not true to begin with. We transform this hand wavy "extreme" to something more quantifiable called the p-value.

Under the Null hypothesis, we assume the true difference in mean is 0 $$ \Delta $$ is 0 and as discussed earlier the sampling distribution will be a normal distribution centered at 0

We do $$ \frac{MLE - truth}{appropriate s.d.} $$ to transform into a standard normal distribution. 

The 2 sample t-test is a very common test to determine whether the difference we observe is real or just due to noise. It is based on the t-statistic  $$ \frac{\Delta}{\sqrt{var(\Delta)}}$$ where $$\Delta = \overline{Y^t} - \overline{Y^c} $$ The T statistic is just the normalized version of $$\delta$$

We compute p-values with the assumption that the t stat follows a normal distribution, further under the null hypothesis it has a mean of 0 and variance 1. Note the normality assumption is not on the sample distribution of Y.

Type 1 error: concluding there is a significant difference when there is no real difference.
A common choice is 5%

p value is the probability that our observed test statistic would be at least as extreme if the null hypothesis is indeed true. (p-value does not capture the probability the Null hypothesis is true given the data observed.

Under null Z follows a standard normal distribution is simply a realization of the squiggle Z which is the standard normal random variable
...
power (1-beta) is the probability of rejecting the null when the null is indeed false. Equivalently, it is the prob of not making a type II error.

|                       | Center Align                      | Right Align                   |
|:-----------           |:------------:                     |------------:                  |
| Don't reject Null     | Correct (1-$$\alpha$$)            | Type II error 100$$\beta$$    |
| Reject Null           | Type I error $$\alpha$$           | Correct (1-$$\beta$$)         |
...
But $$ Z \sim \mathcal{N}(0, 1) $$ is only true when we condition on Null, but not under the alternative. We have to rearrange to obtain something normally distributed.
