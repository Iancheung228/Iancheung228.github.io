...
In ab testing, we have 2 version of a product one is currently in production and the other is the contender. We are interested in determining whether the metric we are interested in is different between the control vs treatment. Many times we can't observe the individual under both treatment and control and we depend on randomization. There is another practical concern: we will rarely conduct the experiment over the entire population, to which we sample a subset of the population for the purpose of the experiment. 

1. sample a subset of population
...
because of step 1, each time we repeat the experiment the observed sample mean will likely be different, a.k.a variability
we can think of the sampled mean as a particular instance of the sampling distribution (can mention CLT)

based on the observed mean (one instance), how unlikely is it to come from the null distribution? If it is extremely unlikely our observed mean is generated by the null distribution, we are likely to suspect the Null hypothesis is not true to begin with. We measure "extreme" by p-value.

Under the Null hypothesis, we assume the true difference in mean is 0 $$ \Delta $$ is 0
as discussed earlier there the sampling distribution will be a normal distribution centered at 0
