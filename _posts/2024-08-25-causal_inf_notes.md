...
In ab tests, we have 2 versions of a product. One is currently in production (control) and the other is the contender (treatment). We wish to determine whether there is a difference between the control vs treatment for a metric that we care about. Most times, we can't observe the individual under both treatment and control and we depend on randomization. There is another practical concern: we will rarely conduct the experiment over the entire population, to which we sample a subset of the population for the experiment. 

High level steps
1. Sample a subset of the population
2. Randomly assign treatment and control to randomization unit
...
From CLT this null distribution will be a normal distribution

We do $$ \frac{MLE - truth}{appropriate s.d.} $$ to transform into a standard normal distribution. 
It is also called the t-statistics  $$ \frac{\Delta}{\sqrt{var(\Delta)}}$$ where $$\Delta = \overline{Y^t} - \overline{Y^c} $$ The T statistic is just the normalized version of $$\delta$$
...
Based on the observed mean, which is one realization from the distribution, we can ask ourselves how unlikely is the sample to come from the null distribution? If it is extremely unlikely our observed mean is generated by the null distribution, we are likely to suspect the Null hypothesis is not true to begin with. We transform this hand-wavy concept of "extreme" to something more quantifiable called the p-value.

P-value is the probability that our observed test statistic would be at least as extreme, under the assumption that the null hypothesis is indeed true. (p-value does not capture the probability the Null hypothesis is true given the data observed.)
...
Under null Z, the observed test statistics comes from a standard normal distribution is simply a realization of the squiggle Z which is the standard normal random variable
