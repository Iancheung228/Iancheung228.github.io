...
P-value is the probability of observing a more extreme test statistic, under the assumption that the null hypothesis is true. $$ P(data | H_0) $$

The Null hypothesis: the null distribution is normally distributed with mean around 0. This distribution is for the r.v $$ \Delta$$, it is a random variable because we runt he ab test on a sample of the population.

A common misconception is the p-value captures the probability the Null hypothesis is true given the data observed. $$ P(H_0 | data) $$
