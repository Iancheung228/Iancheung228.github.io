...
In ab tests, we have 2 versions of a product. One is currently in production (control) and the other is the contender (treatment). We wish to determine whether there is a difference between the control vs treatment on a metric of business concern. Most times, we can't observe the individual under both treatment and control and we depend on randomization. There is another practical concern: we will rarely conduct the experiment over the entire population, to which we sample a subset of the population for the experiment. 
...
Note, due to step 1, each time we repeat the experiment, the observed sample mean and consequently the difference in sample mean will likely vary, a.k.a variability
****we can think of the sampled mean as a particular instance of the sampling distribution (can mention CLT)****

Based on the observed mean (one realization), we can ask ourselves how unlikely is it to come from the null distribution? If it is extremely unlikely our observed mean is generated by the null distribution, we are likely to suspect the Null hypothesis is not true to begin with. We transform this hand-wavy concept of "extreme" to something more quantifiable called the p-value.
